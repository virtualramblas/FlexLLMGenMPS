...
class TorchTensor:
    ...
    def __init__(self, shape, dtype, data, device, name=None):
        '''ORIG if isinstance(data, torch.Tensor):
...
class TorchDevice:
        ...
        if isinstance(k_cache, TorchTensor):
            if attn_sparsity >= 1.0:  # Dense attention
                ...
                if k.is_mps:
                    ...
                    k, v = k.float(), v.float()
                    value = self._attention_value(q, k, v, attention_mask.data,
                        b, src_s, tgt_s, n_head, head_dim).to('mps').half()
    ...
    def _mixed_device_attention(self, q, k_cache, v_cache, k_new, v_new,
        ...
        value = torch.cat([value_gpu, value_cpu.to('mps').half()], dim=0)
        return value
    ...
    def mem_stats(self):
        if self.device_type == DeviceType.MPS:
            cur_mem = torch.mps.current_allocated_memory()
...
def general_copy(dst: TorchTensor, dst_indices: Tuple[slice],
    ...
    elif (src.device.device_type == DeviceType.MPS and
